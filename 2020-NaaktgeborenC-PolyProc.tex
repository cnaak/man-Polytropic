...
\section{Introduction}
    ...
    only holds \emph{local} if the process is such that $n$ is a function of either $P$, $v$, or
    both.
    ...
    local polytropic processes are proposed as discrete building blocks for general  engineering
...
\section{Exact and Local Polytropic Processes}
    ...
    ``local'' polytropic processes, additional constraints  need  to  be  made  to  process  end
    ...
    allied to the  conclusions  drawn  from  Equation~(\ref{eq:affine}),  to  a  polytropic-like
    ...
    \begin{definition}[local polytropic process]\label{def:local.polytropic}
        A local polytropic process is one that  approximates  a  process  in  a  finite  $(P,v)$
    ...
    \begin{conjecture}[general approximability]\label{conj:gen.approx}
        Any continuous, quasi-equilibrium process set of finite complexity can  be  approximated
        within a finite degree of precision by a finite set of local polytropic processes.
    \end{conjecture}

    \noindent\textit{Rationale:\/}~it follows from Definition~\ref{def:local.polytropic} that  a
    finite amount of local polytropic process can approximate, within  suitable  (finite)  error
    intervals, each one of the continuous, quasi-equilibrium process set  of  finite  complexity
    stated on the general approximability conjecture.\vspace{1ex}

    \noindent\textit{Remarks:\/}~it  is  worth  noting  that   all   equilibrium   thermodynamic
    \emph{cycles} of finite complexity can be subdivided into the  process  set  stated  on  the
    ...
    quantities. From the concepts herein defined, local polytropic process seem to be  the  most
    ...
    Conjecture~\ref{conj:gen.approx}, it is proposed that \emph{local polytropic processes},  as
    herein  defined,  to  be  employed  as  \emph{building  blocks}  for   general   engineering
    thermodynamics \emph{process modeling}.
