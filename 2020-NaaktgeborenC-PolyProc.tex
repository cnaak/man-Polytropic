...
\section{Requisites for Exact Polytropic Processes}
    ...
    %---------------------------------------------------------------------------------------
    \subsection{General Substance Model Yielding $u\!:\!u(T)$}
    ...
    peculiar notation\footnote{For  information  on  Bridgman's  relations  and  its  associated
    peculiar  notation,  consult  reference~\cite{2006-BejanA-Wiley}.}.   Therefore,   rewriting
    Eq.~(\ref{eq:uT}) in Bridgman's notation yields
    %
    \begin{equation}
    ...
    compressibility}~\cite{2006-BejanA-Wiley}, also denoted as $\kappa_T$, as to distinguish  it
    from the isentropic compressibility, $\kappa_s$~\cite{1986-JonesJB+HawkinsGA-Wiley}.

    Put differently, any substance for which $\beta T = \kappa P$ will have $u\!:\!u(T)$ only.
    ...
    %
    \begin{equation}
        \delta q - \delta w = du \quad\rightharpoondown\quad (K-1)\delta w = du.
        \label{eq:1stlaw}
    \end{equation}
