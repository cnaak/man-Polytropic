...
\section{Requisites for Exact Polytropic Processes}
    ...
    (ii)~For  overall  Eq.~(\ref{eq:n.deps})'s  second  term  constancy,  with  $v/f(v)  =   1$,
    interactions must cancel the temperature  dependence  of  the  system  substance's  $c_v(T)$
    behavior, resulting in $(1 - K) \propto c_v(T)$, which is hardly practical and is left as  a
    note.
