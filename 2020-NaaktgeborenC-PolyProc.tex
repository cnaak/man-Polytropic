...
\section{Exact and Locally Polytropic Processes}
    ...
    flexible and  natural  way  of  performing  generalized  engineering  thermodynamic  process
    discretizations, suitable for numerical schemes geared towards solving cycles and processes.

    Therefore,      owing      to      the       general       approximability       conjecture,
    Conjecture~\ref{conj:gen.approx},  it  is  herein  proposed  that  \emph{locally  polytropic
    processes} to be used as  \emph{building  blocks}  for  general  engineering  thermodynamics
    \emph{process modeling}.
...
\section{Requisites for Exact Polytropic Processes}
    ...
    \end{align}
    %
    \noindent in which $\mathsf{g}(v)$ is defined by Eq.~(\ref{eq:def.g}). By inspection,  gives
    the polytropic exponent as:
    %
    \begin{align}
    ...
    the only possibility for constancy in $v$ terms. This  result  finally  fixes  the  subtance
