...
\section{Exact and Local Polytropic Processes}
    ...
    allows for the needed flexibility in describing the variety of  transformations  a  material
    working fluid extent---a ``system''---undergo when subject to the various types of  changing
    conditions imposed by engineering  equipments  and  their  subsystems,  as  well  as  purely
    conceptual processes, described only in mathematical terms without association to a  defined
    equipment configuration.

    This lack of end state constraints in the definition of a process  allows  processes  to  be
    splitted into multiple sub-processes, as well as the merging  of  multiple  process  into  a
    super-process.

    Take a ``$1$--$2$''

    Constraining the end states of a process is a necessary condition  to  properly  making  the
    distinction between exact and local polytropic processes.

    Whenever a defined machine configuration is stated

    Thus, under varying degrees  of  idealizations,  machine-specific  processes,  such  as  the
    compression stroke of an internal combustion  engine,  are  modeled  with  quasi-equilibrium
    process.

    can be modeled
    by a quasi-equilibrium

    a ``compression'' process in an internal combustion engine may be
    modeled

    definition of a process

    For the purpose of defining exact and local polytropic processes, 

    Although processes can be described by mathematical relations or functions
