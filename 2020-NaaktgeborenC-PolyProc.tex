...
\section{Requisites for Exact Polytropic Processes}
    ...
    At this point, reference~\cite{2012-ChristiansJ-IntJMechEngEduc} replaces $du$ by  $c_v\,dT$
    and continues the analysis with a $Pv = ZRT$  equation  of  state  with  the  assumption  of
    constant $Z$, based either on ideal gas behavior or  in  processes  of  small  pressure  and
    temperature variation, recall $Z$ is actually $Z:Z(T_r, P_r)$; shortly after, the $ZR$  term
    is replaced by $(c_p - c_v)$, which recovers an ideal gas result.

    The following demonstrates that  assuming  $u:u(T)$  actually  implies  $Z:Z(v)$---a  useful
    intermediate result.
    %
    \begin{equation}
        \label{}
